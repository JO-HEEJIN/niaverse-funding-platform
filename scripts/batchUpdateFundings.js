...
// 사용자 조회 또는 생성
async function ensureUserExists(client, userData) {
  try {
    // 먼저 사용자가 존재하는지 확인
    ...
    if (userResult.rows.length > 0) {
      ...
      console.log(`  기존 사용자: ${userData.email} (ID: ${userId})`);
      return userId;
    }
    
    // 사용자가 없으면 생성 (엑셀의 실제 데이터 사용)
    console.log(`  새 사용자 생성: ${userData.email}`);
    const newUserResult = await client.query(
      `INSERT INTO users (email, password_hash, name, phone, confirmed, is_admin) 
       VALUES ($1, $2, $3, $4, true, false) 
       RETURNING id`,
      [
        userData.email,
        '$2a$10$dummy.hash.for.batch.import', // 더미 비밀번호 해시
        userData.name || userData.email.split('@')[0], // 엑셀의 이름 또는 이메일 앞부분
        userData.phone || '010-0000-0000' // 엑셀의 연락처 또는 기본값
      ]
    );
    
    if (newUserResult.rows.length === 0) {
      throw new Error(`사용자 생성 실패: ${userData.email}`);
    }
    
    const userId = newUserResult.rows[0].id;
    console.log(`  사용자 생성 완료: ${userData.email} (ID: ${userId})`);
    return userId;
    
  } catch (error) {
    console.error(`  사용자 처리 오류 (${userData.email}):`, error.message);
    throw error;
  }
}

// DB 업데이트 함수
async function updateUserFundings(client, userData, consolidatedFundings) {
  const updatePromises = [];
  
  // 먼저 사용자 ID 확인
  const userId = await ensureUserExists(client, userData);
  
  for (const [fundingId, fundingData] of consolidatedFundings) {
    try {
      ...
      if (existingResult.rows.length > 0) {
        ...
        console.log(`  - ${fundingId} 업데이트: ${price}원, 수익: ${fundingData.totalAccumulated} ${fundingData.unit}`);
      } else {
        // 신규 삽입
        await client.query(
          `INSERT INTO purchases (user_id, funding_id, quantity, price, accumulated_income, created_at, updated_at)
           VALUES ($1, $2, $3, $4, $5, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)`,
          [userId, fundingId, quantity, price.toString(), fundingData.totalAccumulated.toString()]
        );
        console.log(`  - ${fundingId} 신규 추가: ${price}원, 수익: ${fundingData.totalAccumulated} ${fundingData.unit}`);
      }
      
    } catch (error) {
      console.error(`  - ${fundingId} 처리 실패:`, error.message);
      throw error;
    }
  }
}
...
// 업데이트 로그 저장
function saveUpdateLog(results) {
  const logData = {
  ...
  try {
    ...
    for (const [email, userData] of userFundingMap) {
      try {
        ...
        // DB 업데이트
        await updateUserFundings(client, userData, consolidated);
        
        tracker.update(email, 'SUCCESS');
        results.push({ email, status: 'success', fundings: Array.from(consolidated.keys()) });
